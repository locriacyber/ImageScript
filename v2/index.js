...
const wasm = {
  ...
  tiff: require('../wasm/node/tiff.js') // font: require('../wasm/node/font.js'),
...
class Image extends framebuffer {
  ...
  encode(format, options = {}) {
    ...
    if ('png' === format) return codecs.png[method](this.u8, { ...options,
      width: this.width,
      height: this.height
    });
    if ('jpeg' === format) return codecs.jpeg[method](this.u8, { ...options,
      width: this.width,
      height: this.height
    });
    if ('webp' === format) return codecs.webp[method](this.u8, { ...options,
      width: this.width,
      height: this.height
    });
  ...
  static async decode(format, buffer, options = {}) {
    ...
    let frame;
    if ('svg' === format) frame = (await wasm.svg.init()).load(buffer, options.fit ?? null);else if (format in wasm) frame = (await wasm[format].init()).load(buffer);
...
class Animation {
  ...
  add(frame) {
    if (this.width !== frame.width) throw new RangeError('invalid frame width');else if (this.height !== frame.height) throw new RangeError('invalid frame height');
  ...
  async encode(format, options = {}) {
    if ('gif' === format) {
      ...
      return encoder.finish({ ...options,
        repeat: Infinity === repeat ? null : repeat
      });
    }
