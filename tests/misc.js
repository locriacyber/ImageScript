...
if (Image.hslaToColor(0, 0, 1, 0xff) !== 0xffffffff) panic('hslaToColor for s=0 failed');
{
  for (const rgba of [0xff0000ff, 0x00ff00ff, 0x0000ffff, 0x000000ff, 0xffffffff].map(x => Image.colorToRGBA(x))) {
    ...
    for (let i = 0; i < 4; i++) if (rgba[i] !== rgbaBacktransform[i]) panic('rgbaToHSLA failed');
  }
}
{
  ...
  for (let i = 0; i < 3; i++) if (rgb[i] !== target[i]) panic('colorToRGB failed');
}
{
  const image = new Image(1, 1);
  image.setPixelAt(0, 0, 0xff8000ff);
  const pixel = image.getPixelAt(0, 0);
  if (pixel !== 0xff8000ff) panic('getPixelAt fails');
  const rgba = image.getRGBAAt(0, 0);
  const target = [0xff, 0x80, 0x00, 0xff];

  for (let i = 0; i < 4; i++) if (rgba[i] !== target[i]) panic('colorToRGB failed');
}
{
  const image = new Image(1, 1);

  try {
    image.__check_boundaries__('a', 0);

    panic('check boundaries for x as NaN failed');
  } catch {}

  try {
    image.__check_boundaries__(0, 'a');
  ...
  try {
    ...
    panic('check boundaries for x < failed');
  } catch {}

  try {
    image.__check_boundaries__(0, -1);

    panic('check boundaries for y < failed');
  } catch {}
...
}
{
  ...
  if (!Buffer.from(clone.bitmap).equals(Buffer.from(image.bitmap))) panic('clone failed');
}
{
  const image = new Image(2, 2);
  image.scale(.5);
  if (image.width !== 1 || image.height !== 1) panic('resize failed');
  image.scale(1);
  if (image.width !== 1 || image.height !== 1) panic('resize failed');
...
}
{
  ...
  if (image.getPixelAt(0, 0) !== 0xff8000ff) panic('static drawBox failed');
  image.drawBox(1, 1, 1, 1, () => 0x00ff00ff);
  if (image.getPixelAt(0, 0) !== 0x00ff00ff) panic('fn drawBox failed');
}
{
  const image = new Image(1, 1);
  const toFail = ['a', -1];
  ...
  for (const func of funcs) for (const value of toFail) try {
  ...
  for (const func of funcs) for (const value of toPass) func.call(image, ...value);
}
{
