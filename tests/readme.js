...
(async () => {
  const [backgroundSVG, avatarBinary, badges, font] = await Promise.all([fs.readFile('./tests/svgs/background.svg').then(b => b.toString()), fs.readFile('./tests/targets/external.png'), Promise.all(['crown', 'potato', 'mask', 'microbe', 'petri_dish', 'thermometer', 'cigarette'].map(x => fs.readFile(`./tests/svgs/${x}.svg`).then(b => b.toString()))), fs.readFile('./tests/fonts/carbon phyber.ttf')]);
  ...
  const avatarGradient = Image.gradient({
    0: 0x00ffffff,
    1: 0x0080ffff
  });
  ...
  image.composite(discriminator, 0.3 * image.width, 0.05 * image.height + username.height); // Global XP

  {
    ...
    const xpGradient = Image.gradient({
      0: 0x0080ffff,
      .8: 0x00ffffff,
      1: 0x00ff80ff
    });
  ...
  if (process.env.OVERWRITE_TEST) await fs.writeFile('./tests/targets/readme.png', encoded);
  if (!(await fs.readFile('./tests/targets/readme.png')).equals(Buffer.from(encoded))) process.exit(1);
})();