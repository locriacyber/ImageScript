...
import { default as v2 } from './v2/framebuffer.js'; // old
...
class Image {
  ...
  *iterateWithColors() {
  ...
  __check_boundaries__(x, y) {
    ...
    if (x < 0) throw new RangeError(`${Image.__out_of_bounds__} (x=${x})<0`);
    if (x >= this.width) throw new RangeError(`${Image.__out_of_bounds__} (x=${x})>=(width=${this.width})`);
    if (y < 0) throw new RangeError(`${Image.__out_of_bounds__} (y=${y})<0`);
    if (y >= this.height) throw new RangeError(`${Image.__out_of_bounds__} (y=${y})>=(height=${this.height})`);
  }
  /**
   * @private
   */
  ...
  __resize__(width, height, mode = Image.RESIZE_NEAREST_NEIGHBOR) {
    if (width === Image.RESIZE_AUTO && height === Image.RESIZE_AUTO) throw new Error('RESIZE_AUTO can only be used for either width or height, not for both');else if (width === Image.RESIZE_AUTO) width = this.width / this.height * height;else if (height === Image.RESIZE_AUTO) height = this.height / this.width * width;
    width = Math.floor(width);
    height = Math.floor(height);
    if (width < 1) throw new RangeError('Image has to be at least 1 pixel wide');
    if (height < 1) throw new RangeError('Image has to be at least 1 pixel high');
    let image;
    if (mode === Image.RESIZE_NEAREST_NEIGHBOR) image = this.__resize_nearest_neighbor__(width, height);else throw new Error('Invalid resize mode');
    return image;
  }
  /**
   * @private
  ...
  drawCircle(x, y, radius, color) {
    ...
    for (let currentY = Math.max(1, y - radius); currentY <= Math.min(y + radius, this.height); currentY++) {
      for (let currentX = Math.max(1, x - radius); currentX <= Math.min(x + radius, this.width); currentX++) {
        if ((currentX - x) ** 2 + (currentY - y) ** 2 < radSquared) this.__set_pixel_1_indexed__(currentX, currentY, typeof color === 'function' ? color(currentX - x + radius, currentY - y + radius) : color);
      }
    }
  ...
  invert() {
    for (const [x, y, color] of this.iterateWithColors()) this.__set_pixel_1_indexed__(x, y, 0xffffffff - color & 0xffffff00 | color & 0xff);
  ...
  static gradient(colors) {
    ...
    if (positions.length === 0) throw new RangeError('Invalid gradient point count');else if (positions.length === 1) {
      return () => values[0];
  ...
  roundCorners(radius = Math.min(this.width, this.height) / 4) {
    ...
    for (let x = 1; x <= radius; x++) {
      ...
      for (let y = this.height - radius; y <= this.height; y++) {
        if (xRad + (this.height - y - radius) ** 2 > radSquared) this.bitmap[((y - 1) * this.width + x - 1) * 4 + 3] = 0;
      }
    }
  ...
  static async decode(data) {
    ...
    if (ImageType.isPNG(view)) {
      ...
      if (pixelType === 0) {
        ...
        for (let i = 0; i < width * height; i++) image.bitmap.set(buffer.subarray(i * 3, i * 3 + 3), i * 4);
      } else if (pixelType === 2) {
  ...
  static async renderSVG(svg, size = 1, mode = this.SVG_MODE_SCALE) {
    if (![this.SVG_MODE_WIDTH, this.SVG_MODE_HEIGHT, this.SVG_MODE_SCALE].includes(mode)) throw new Error('Invalid SVG scaling mode');
...
class Frame extends Image {
  ...
  static __convert_disposal_mode__(mode) {
    if (typeof mode === 'string') mode = ['any', 'keep', 'previous', 'background'].indexOf(mode);
  ...
  constructor(width, height, duration = 100, xOffset = 0, yOffset = 0, disposalMode = Frame.DISPOSAL_KEEP) {
    if (isNaN(duration) || duration < 0) throw new RangeError('Invalid frame duration');
  ...
  *[Symbol.iterator]() {
  ...
  async encode(quality = 95) {
    ...
    return encoder.finish({
      repeat: -1 === this.loopCount ? null : this.loopCount
    });
  }
  /**
   * Decodes a GIF image
...
class TextLayout {
  ...
  constructor(options) {
    ...
    if (!['word', 'char'].includes(this.wrapStyle)) throw new RangeError('Invalid wrapStyle');
    this.verticalAlign = verticalAlign || 'left';
    if (!['left', 'center', 'right'].includes(this.verticalAlign)) throw new RangeError('Invalid verticalAlign');
    this.horizontalAlign = horizontalAlign || 'top';
    if (!['top', 'middle', 'bottom'].includes(this.horizontalAlign)) throw new RangeError('Invalid horizontalAlign');
